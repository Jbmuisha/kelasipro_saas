...
import '@/app/dashboard/school/messages/messages.css';
...
export default function TeacherMessagesPage() {
  ...
  const fetchConversation = async (contactId: number) => {
    try {
      ...
      if (!res.ok) {
        // Log but don't throw – this runs on a 5s poll and transient failures are normal
    ...
    <div className="messages-container">
      <div className="messages-sidebar">
        <div className="messages-sidebar-header">
          <h2>💬 Messages</h2>
          <input className="messages-search" type="text" placeholder="Search contacts..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
        <div className="messages-contacts-list">
          ...
          {filteredContacts.map(c => (
            <div key={c.id} className={`messages-contact ${selectedContact?.id === c.id ? 'active' : ''}`} onClick={() => { setSelectedContact(c); setError(null); }}>
              <div className="contact-avatar-wrapper">
                <div className={avatarBg(c.role)}>{c.name.charAt(0).toUpperCase()}</div>
          ...
          <>
            <div className="chat-header">
              <div className="chat-header-avatar-wrapper">
                <div className="chat-header-avatar" style={{ background: headerBg(selectedContact.role) }}>{selectedContact.name.charAt(0).toUpperCase()}</div>
            ...
            <div className="chat-messages">
              ...
              {messages.length === 0 && (
                <div className="chat-no-messages"><span style={{ fontSize: 40 }}>👋</span><span>No messages yet. Say hello!</span></div>
              )}
                    ...
                    return (
                      <div key={m.id} className={`chat-bubble-row ${isMine ? 'mine' : 'theirs'}`}>
                        <div className={`chat-bubble ${isMine ? 'mine' : 'theirs'}`}>
                          <div>{m.content}</div>
                          <div className="chat-bubble-time">{m.created_at ? new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="chat-input-area">
              <textarea className="chat-input" value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={handleKeyDown} placeholder="Type a message..." rows={1} />
              <button className="chat-send-btn" onClick={sendMessage} disabled={sending || !newMessage.trim()} title="Send">➤</button>
            </div>
          </>
        )}
      </div>
